    ...
    // const svg = document.querySelector('svg');
     ...
     svgpaths.forEach(function(svgPath){
          svgPath.addEventListener('mouseover', (event) => {
               vibStart(event.target);
          });
          svgPath.addEventListener('mouseleave', (event) => {
             vibStop(event.target);   
          });
     });
...
function vibStart(pathid) {
         ...
         /*for (var i = 0; i<svgpath.length +1; i += 1) {
              if (svgpath.id == "path" + i) {
                              vibrate();
                              if ('speechSynthesis' in window) {
                                msg.text = document.querySelector('desc'+ i).firstChild.data;
                                speechSynthesis.speak(msg);
                                msg.addEventListener('end', function () {
                                    speechSynthesis.cancel();
                                });
